...
class MotherBoard:
    def __init__(self, name: str, cpu, *memory):
        self.name = name
        self.cpu = cpu
        self.total_mem_slots = 4    # число слотов памяти всегда = 4
        self.mem_slots = memory[:self.total_mem_slots]      # используем срез, число переданной пямяти было <= 4
    ...
    def get_config(self):
        info = [f'Материнская плата: {self.name}',
                ...
                # если не понимаешь ↑ конструкцию, смотри https://stepik.org/course/100707/syllabus - 9 модуль
        return info


mb = MotherBoard('MSI', CPU('Ryzen 5 1600', 3.5), Memory('Kingston', volume=8000), Memory('Kingston', volume=8000))